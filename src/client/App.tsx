...
function App() {
  ...
  }, [token, view, page, statusFilter, searchQuery]);

  const fetchData = async () => {
    try {
      if (view === 'jobs') {
        const res = await api.get(`/jobs?page=${page}&status=${statusFilter}&search=${searchQuery}`);
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
                  <p className="text-slate-500 text-sm font-medium mb-1">Total Processed</p>
              ...
              <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl">
                ...
                <div className="space-y-4">
                  {stats.topCompanies?.map((c: any) => (
                    <div key={c.company} className="flex items-center justify-between p-4 bg-slate-950/50 rounded-xl border border-slate-800/50">
                      <span className="font-medium text-slate-200">{c.company}</span>
                      <span className="bg-indigo-600/20 text-indigo-400 px-3 py-1 rounded-full text-sm font-bold">{c.count} Listings</span>
                    </div>
                  ))}
                </div>
              </div>
